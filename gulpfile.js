...
function pluginsCss(done) {
    gulp.src(src.pluginsCss)
        .pipe(concat('plugins/plugins.min.css'))
        .pipe(cleanCSS())
...
function vendorCss(done) {
    gulp.src(src.vendorCss)
        .pipe(concat('vendor/vendor.min.css'))
        .pipe(cleanCSS())
        .pipe(gulp.dest(dest.css));
    done();
}
...
gulp.task('allTask', gulp.series(html, assets, pluginsCss, vendorCss, styleCss, pluginsJs, vendorJs, mainJs));
